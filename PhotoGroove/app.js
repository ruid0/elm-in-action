			...
			default:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							selectedPhoto: $author$project$PhotoGroove$getPhotoUrl(index)
						}),
					$elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$PhotoGroove$view = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('photo-groove')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$src($author$project$PhotoGroove$urlPrefix + ('large/' + model.selectedPhoto))
					]),
				_List_Nil)
			]));
};
